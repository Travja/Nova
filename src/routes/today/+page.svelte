<script lang="ts">
	...
	import { focusForToday, formatAmount, formatTimeLeft } from '$domain/progress';
...
<section class="today">
	<header class="head">
		<div>
			...
			<p class="muted">
				...
				{:else}
					{focus.atRisk.length}
					{focus.atRisk.length === 1 ? 'orbit needs' : 'orbits need'} attention before the period closes.
				{/if}
			</p>
		</div>
	...
	{:else if focus.atRisk.length === 0}
		<div class="empty panel">
			...
			<h2>Clear sky</h2>
			<p class="muted">
				Nothing is short of target with its period closing. Whatever is still in flight has room
				left.
			</p>
		</div>
	{:else}
		<ul class="risk">
			{#each focus.atRisk as snapshot (snapshot.goal.id)}
				<li>
					<p class="deadline">
						<span class="pill" style="color: {TIER_DEFINITIONS[snapshot.goal.tier].accent}">
							{TIER_DEFINITIONS[snapshot.goal.tier].label}
						</span>
						<span class="muted"
							>{timeLeft(snapshot)} of {CADENCE_LABEL[snapshot.current.period.cadence]}</span
						>
					</p>
	...
	{#if focus.steady.length > 0}
		<details class="fold">
			...
			<p class="muted fold__note">
				Still in flight with room left, the one furthest behind first. Nothing here is owed today.
			</p>
			<ul class="compact">
				{#each focus.steady as snapshot (snapshot.goal.id)}
					<li>
						<a href={resolve('/goals/[id]', { id: snapshot.goal.id })}>{snapshot.goal.title}</a>
						<span class="muted">{standing(snapshot)}</span>
						<span class="muted">{timeLeft(snapshot)}</span>
					</li>
				{/each}
			</ul>
		</details>
